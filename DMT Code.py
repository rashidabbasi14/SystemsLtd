...
def main():
    # User Data
    wb = load_workbook(filename = 'Input/UserSheet.xlsx')
    ws = wb.active
    ...
    for col in ws.iter_cols(min_row = minrow, min_col = mincol, max_col = maxcol, max_row = maxrow):
        for cell in col:
            ...
            nws['K'+str(cell.row)] = ws['J'+str(cell.row)].value if datetime.strptime(str(ws['J'+str(cell.row)].value), '%Y%m%d') > datetime.today() else datetime.today().strftime('%Y%m%d')
            nws['L'+str(cell.row)] = "00:00"
            nws['M'+str(cell.row)] = "00:00" if str(ws['L'+str(cell.row)].value) == "0" else "24:00"
            ...
            nws['Z'+str(cell.row)] = ws['M'+str(cell.row)].value
            nws['AA'+str(cell.row)] = "00"
            nws['AB'+str(cell.row)] = "?."
            nws['AC'+str(cell.row)] = "ALL.COMPANY"
            nws['AD'+str(cell.row)] = "1"
    ...
    nwb.save('Output/USER DMT.xlsx')
    bwb.save('Output/BP DMT.xlsx')
    save_error_files()
    create_csv()
...
def init_headers(nws, bws):
    ...
    nws['Z1'] = "USER.STATUS"
    nws['AA1'] = "DEALER.DESK"
    nws['AB1'] = "AMOUNT.FORMAT"
    nws['AC1'] = "OTH.BOOK.ACCESS"
    nws['AD1'] = "DATE.FORMAT"
...
def create_csv():
    ...
    wb.close()
    os.remove('Output/USER DMT.xlsx')
    os.remove('Output/BP DMT.xlsx')
